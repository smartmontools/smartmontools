...
const char * dev_qnap_tr004_cpp_svnid = "$Id$";

static void jmbassert_failed(int line, const char * expr)
{
  char msg[128];
  // Avoid __FILE__ as it may break reproducible builds
  snprintf(msg, sizeof(msg), "dev_qnap_tr004.cpp(%d): Assertion failed: %s", line, expr);
  ...
  const uint8_t * cmd, unsigned cmdsize)
{
  ...
  jmb_put_le32(data, 0, 0x197b0393); // SCRAMLED_CMD
...
namespace qnap_tr004 {

class qnap_tr004_device
...
{
public:
  qnap_tr004_device(smart_interface * intf, smart_device * smartdev, const char * req_type,
    uint8_t port, uint8_t lba, bool force);

  virtual ~qnap_tr004_device() throw();
...
qnap_tr004_device::qnap_tr004_device(smart_interface * intf, smart_device * smartdev, const char * req_type,
  uint8_t port, uint8_t lba, bool force)
...
{
  set_info().info_name = strprintf("%s [qnap_tr004_disk_%u]", smartdev->get_info_name(), port);
  memset(m_orig_data, 0, sizeof(m_orig_data));
}

qnap_tr004_device::~qnap_tr004_device() throw()
{
  if (m_orig_write_back) try {
    qnap_tr004_device::restore_orig_data();
  } catch (...) {
    // ignore
  }
}

bool qnap_tr004_device::raw_read(uint8_t (& data)[512])
{
  memset(data, 0, sizeof(data));
...
bool qnap_tr004_device::raw_write(const uint8_t (& data)[512])
{
  if (get_tunnel_dev()->is_scsi()) {
...
bool qnap_tr004_device::run_jmb_command(const uint8_t * cmd, unsigned cmdsize, uint8_t (& response)[512])
{
  // Set up request
  uint8_t request[512];
  ...
  if (ata_debugmode) {
    pout("qnap_tr004: Write request sector #%d\n", m_cmd_id);
    if (ata_debugmode > 1)
  ...
  if (ata_debugmode) {
    pout("qnap_tr004: Write obfuscated request sector #%d\n", m_cmd_id);
    if (ata_debugmode > 1)
      dStrHex(request, sizeof(request), 0);
  }
  ...
  if (ata_debugmode) {
    pout("qnap_tr004: Read response sector #%d\n", m_cmd_id);
    if (ata_debugmode > 1)
  ...
  if (!memcmp(request, response, sizeof(request))) { // regular I/O?
    m_blocked = true;
    return set_err(EIO, "No qnap_tr004 response detected");
  }
  if (!jmb_check_crc(response)) {
    m_blocked = true;
    jmb_xor(response);
    return set_err(EIO, "%s", (!jmb_check_crc(response)
      ? "CRC error in qnap_tr004 response"
      : "qnap_tr004 response contains a wakeup sector"));
  }
  if (memcmp(request, response, 8)) { // code + id identical?
    m_blocked = true;
    return set_err(EIO, "Invalid header in qnap_tr004 response");
  }
...
void qnap_tr004_device::report_orig_data_lost() const
{
  bool zf = !nonempty(m_orig_data, sizeof(m_orig_data));
  pout("qnap_tr004: WARNING: Data (%szero filled) at LBA %d lost\n", (zf ? "" : "not "), m_lba);
  if (!zf) // Dump lost data
    dStrHex(m_orig_data, sizeof(m_orig_data), 0);
}

bool qnap_tr004_device::restore_orig_data()
{
  if (ata_debugmode)
    pout("qnap_tr004: Restore original sector (%szero filled)\n",
...
bool qnap_tr004_device::open()
{
  ...
  // Read original data
  if (ata_debugmode)
    pout("qnap_tr004: Read original data at LBA %d\n", m_lba);
    ...
    if (!m_force) {
      ...
      return set_err(EINVAL, "Original sector at LBA %d %s", m_lba,
        (st == 0 ? "is not zero filled" :
         st == 1 ? "contains qnap_tr004 wakeup data"
                 : "contains qnap_tr004 protocol data"));
    }
    if (st) {
      // Zero fill to reset protocol state
      if (ata_debugmode)
        pout("qnap_tr004: Zero filling original data\n");
      memset(m_orig_data, 0, sizeof(m_orig_data));
    }
  }
  ...
  for (int id = 0; id < 4; id++) {
    ...
    if (ata_debugmode) {
      pout("qnap_tr004: Write wakeup sector #%d\n", id+1);
      if (ata_debugmode > 1)
    ...
    if (!raw_write(dataout)) {
        ...
        tunnelled_device<ata_device, smart_device>::close();
        m_blocked = true;
        return set_err(err.no, "Write of qnap_tr004 wakeup sector #%d: %s", id + 1, err.msg.c_str());
    }
  }
  ...
  if (response[16] < ' ') {
    close();
    return set_err(ENOENT, "No device connected to qnap_tr004 port %d", m_port);
  }

  return true;
}

bool qnap_tr004_device::close()
{
  bool ok = true;
...
bool qnap_tr004_device::ata_pass_through(const ata_cmd_in & in, ata_cmd_out & /* out */)
{
  jmbassert(is_open());
  if (m_blocked)
    return set_err(EIO, "Device blocked due to previous errors");
  if (in.direction == ata_cmd_in::no_data) // TODO: add to ata_cmd_is_supported() ?
    return set_err(ENOSYS, "NO DATA ATA commands not implemented [qnap_tr004]");
  if (!ata_cmd_is_supported(in, 0, "qnap_tr004"))
    return false;
  // Block all commands which require full sector data
  int supported = is_supported_by_jmb(in.in_regs);
  if (!supported)
    return set_err(ENOSYS, "ATA command not implemented due to truncated response [qnap_tr004]");
  ...
  if (status == 0x00) {
    m_blocked = true;
    return set_err(EIO, "No device connected to qnap_tr004 port %d", m_port);
  }
...
ata_device * smart_interface::get_qnap_tr004_device(const char * type, smart_device * smartdev)
{
  ...
  sscanf(type, "qnaptr,%u%n", &port, &n1);
  ...
  if (!(n1 == len && port <= 4 && 33 <= lba && lba <= 62)) {
    set_err(EINVAL, "Option -d qnaptr,N[,sLBA][,force] must have 0 <= N <= 4 [, 33 <= LBA <= 62]");
    return 0;
  }

  ata_device * jmbdev = new qnap_tr004::qnap_tr004_device(this, smartdev, type, port, lba, force);
