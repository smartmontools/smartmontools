...
ver=$(sed -n 's|^AC_INIT[^,]*, *\[\([0-9.]*\)\] *,.*$|\1|p' "$srcdir/configure.ac") || exit 1
test -n "$ver" || error "$srcdir/configure.ac: package version not found"
...
case $x in
  20*) ;;
  \ \#*) pre="pre-" ;;
  *) error "$srcdir/configure.ac: unable to detect pre-release state" ;;
esac

# Check whether this is a git checkout
is_git_co=true
test -n "$(cd "$srcdir" && git ls-files README.md 2>/dev/null)" || is_git_co=false

# Check for the copy of 'version.sh' generated by 'make dist'
dist_version_sh="$srcdir/util/dist-version.sh"
...
if $is_git_co; then
  ...
  x="$(cd "$srcdir" && git status -s -uno)" || exit 1
  modified=${x:+-modified}
  rev="$rev$modified"

  # Emulate a svn revision number
  if x=$(cd "$srcdir" && git rev-list --count "$base_git_rev..$head" 2>/dev/null) \
     && [ "$x" -gt 0 ]; then
  ...
  if [ -n "$pre" ]; then
    ...
    if [ -n "$prev_release" ]; then
      ...
      if x=$(cd "$srcdir" && git rev-list --count "$prev_release..$head" 2>/dev/null)
      then
